    ...
    with get_connection() as conn, conn.cursor() as cur:
        ...
        cur.execute(
            ...
            CREATE TABLE IF NOT EXISTS surgeries (
                ...
                day3_asked BOOLEAN NOT NULL DEFAULT FALSE,
                day3_sent BOOLEAN NOT NULL DEFAULT FALSE,
                day7_sent BOOLEAN NOT NULL DEFAULT FALSE,
                reminder_sent BOOLEAN NOT NULL DEFAULT FALSE,
                UNIQUE (phone, surgery_date)
            )
            """
        )
        cur.execute("ALTER TABLE surgeries ADD COLUMN IF NOT EXISTS reminder_sent BOOLEAN NOT NULL DEFAULT FALSE")
        conn.commit()


def upsert_patient(chat_id: int, full_name: str, username: str | None, phone: str | None = None) -> None:
    with get_connection() as conn, conn.cursor() as cur:
        if phone:
            cur.execute(
                """
                INSERT INTO patients (chat_id, full_name, username, phone)
                VALUES (%s, %s, %s, %s)
                ...
                """,
                (chat_id, full_name, username, phone),
            )
        else:
            cur.execute(
                """
...
def get_patient_by_phone(phone: str) -> int | None:
    with get_connection() as conn, conn.cursor() as cur:
        cur.execute("SELECT chat_id FROM patients WHERE phone = %s", (phone,))
        row = cur.fetchone()
...
def upsert_surgery(phone: str, full_name: str, surgery_date: date) -> None:
    with get_connection() as conn, conn.cursor() as cur:
        cur.execute(
            """
            INSERT INTO surgeries (phone, full_name, surgery_date)
            VALUES (%s, %s, %s)
            ON CONFLICT (phone, surgery_date) DO UPDATE SET full_name = EXCLUDED.full_name
            """,
            (phone, full_name, surgery_date),
        )
        conn.commit()


def get_surgeries_due_for_day3(threshold: date) -> list[tuple]:
    with get_connection() as conn, conn.cursor() as cur:
        cur.execute(
            """
            SELECT id, phone, full_name, surgery_date, reminder_sent FROM surgeries
            WHERE day3_asked = FALSE AND surgery_date <= %s
            """,
            (threshold,),
        )
        return cur.fetchall()
...
def get_surgeries_due_for_day7(threshold: date) -> list[tuple]:
    with get_connection() as conn, conn.cursor() as cur:
        cur.execute(
            """
            SELECT id, phone, full_name, surgery_date FROM surgeries
            WHERE day3_sent = TRUE AND day7_sent = FALSE AND surgery_date <= %s
            """,
            (threshold,),
        )
        return cur.fetchall()
...
def get_surgery(surgery_id: int) -> tuple | None:
    with get_connection() as conn, conn.cursor() as cur:
        cur.execute("SELECT id, phone, full_name, surgery_date FROM surgeries WHERE id = %s", (surgery_id,))
        return cur.fetchone()


def mark_day3_asked(surgery_id: int) -> None:
    with get_connection() as conn, conn.cursor() as cur:
        cur.execute("UPDATE surgeries SET day3_asked = TRUE WHERE id = %s", (surgery_id,))
        conn.commit()


def mark_day3_sent(surgery_id: int) -> None:
    with get_connection() as conn, conn.cursor() as cur:
        cur.execute("UPDATE surgeries SET day3_sent = TRUE WHERE id = %s", (surgery_id,))
        conn.commit()


def mark_day7_sent(surgery_id: int) -> None:
    with get_connection() as conn, conn.cursor() as cur:
        cur.execute("UPDATE surgeries SET day7_sent = TRUE WHERE id = %s", (surgery_id,))
        conn.commit()


def mark_reminder_sent(surgery_id: int) -> None:
    with get_connection() as conn, conn.cursor() as cur:
        cur.execute("UPDATE surgeries SET reminder_sent = TRUE WHERE id = %s", (surgery_id,))
        conn.commit()
