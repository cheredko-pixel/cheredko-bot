...
from db import get_language, init_db, save_checkin, set_language, upsert_patient
...
async def start_checkin(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args:
        await update.message.reply_text("Формат: /checkin <ID пациента>")
        return
    ...
    await context.bot.send_message(patient_chat_id, t("checkin_intro", lang))
